...
# CHANGE THIS:
BASE_URL="https://github.com/bustermacintosh/secureerase/releases/latest/download"

TMP="${TMPDIR:-/tmp}/secureerase.$$"
...
if [ "$(/usr/sbin/sysctl -n hw.optional.arm64 2>/dev/null)" = "1" ]; then
    ...
    BINARY="secureerase-arm64"
else
    if [ "$OS_MAJOR" -lt 12 ]; then
        echo "This requires macOS Monterey 12 or newer on Intel."
        exit 1
    fi
    BINARY="secureerase-amd64"
fi

echo "Downloading secureerase..."
